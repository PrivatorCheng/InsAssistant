...
let currentPolicyInsureTypeList = [];
...
const DEBUG_PANEL_MAX_DISPLAY_CHARS = 200;
 ...
 * @returns {HTMLDivElement|null} radio 群組容器
 */
function getLlmProviderGroup() {
    return document.getElementById("llmProviderGroup");
}

/**
 * 取得目前選取的 LLM 提供者。
 * @returns {string|null} LLM 提供者
 */
function getSelectedLlmProvider() {
    var _a, _b;
    const checked = document.querySelector('input[name="llmProviderOption"]:checked');
    return ((_b = (_a = checked === null || checked === void 0 ? void 0 : checked.value) === null || _a === void 0 ? void 0 : _a.trim()) !== null && _b !== void 0 ? _b : "") || null;
}
...
function setSelectedModel(model) {
    ...
    modelText.textContent = (model === null || model === void 0 ? void 0 : model.trim()) || "-";
}

function setQuerySummary(summary) {
    const summaryText = document.getElementById("querySummaryText");
    if (!summaryText) {
        return;
    }

    summaryText.textContent = (summary === null || summary === void 0 ? void 0 : summary.trim()) || "";
}

function getStorageUser() {
    const raw = window.sessionStorage.getItem(TEXT_TO_SQL_AUTH_STORAGE_KEY);
    if (!raw) {
        return null;
    }
    ...
    try {
        ...
        if (!(parsed === null || parsed === void 0 ? void 0 : parsed.userId) || !(parsed === null || parsed === void 0 ? void 0 : parsed.userName)) {
            return null;
        }
...
function isVoiceOutputEnabled() {
    const checkbox = document.getElementById("voiceOutputEnabled");
    return checkbox instanceof HTMLInputElement && checkbox.checked;
}

function setVoiceOutputStatus(message) {
    const statusElement = document.getElementById("voiceOutputStatusText");
    if (statusElement) {
        statusElement.textContent = message;
    }
}

async function playVoiceText(text, forceEnabled, saveToDocTestWaveFile) {
    const normalizedText = (text || "").toString().trim();
    if (!normalizedText) {
        return;
    }

    if (!forceEnabled && !isVoiceOutputEnabled()) {
        setVoiceOutputStatus("語音輸出未啟用");
        return;
    }
...
async function queryInsuranceChat(sessionId, userMessage, llmProvider, promptTemplateMode) {
    ...
    const payload = {
        ...
        policyInsureTypeList: selectedMode === PROMPT_TEMPLATE_MODE_CLAIM_ASSISTANT
            ? (Array.isArray(currentPolicyInsureTypeList)
                ? currentPolicyInsureTypeList.filter((item) => (item || "").toString().trim().length > 0)
                : [])
            : []
    };
...
function setCurrentPolicyInsureTypeList(value) {
    currentPolicyInsureTypeList = normalizePolicyInsureTypeList(value);
}
...
function appendKeywordSystemPromptDebug(keywordSystemPrompt) {
    const promptList = document.getElementById("llmKeywordSystemPrompt");
    setDebugTextWithCopySource(promptList, keywordSystemPrompt, DEBUG_PANEL_MAX_DISPLAY_CHARS);
}

function appendKeywordDebug(keywordText) {
    const keywordBlock = document.getElementById("llmKeywordResult");
    setDebugTextWithCopySource(keywordBlock, keywordText, DEBUG_PANEL_MAX_DISPLAY_CHARS);
}
...
function appendLlmLogErrorsDebug(llmLogErrors) {
    ...
    if (!Array.isArray(llmLogErrors) || llmLogErrors.length === 0) {
        setDebugTextWithCopySource(errorListBlock, "", DEBUG_PANEL_MAX_DISPLAY_CHARS);
        return;
    }
    ...
    const fullText = normalizedErrors
        .map((item, index) => `${index + 1}. ${item}`)
        .join("\n");

    setDebugTextWithCopySource(errorListBlock, fullText, DEBUG_PANEL_MAX_DISPLAY_CHARS);
}
...
function appendLlmResultDebug(llmResult) {
    ...
    if (llmResult === null || llmResult === undefined) {
        setDebugTextWithCopySource(resultBlock, "", DEBUG_PANEL_MAX_DISPLAY_CHARS);
        return;
    }

    if (typeof llmResult === "string") {
        const normalizedResult = llmResult.trim();
        setDebugTextWithCopySource(resultBlock, normalizedResult, DEBUG_PANEL_MAX_DISPLAY_CHARS);
        return;
    }
    ...
    try {
        ...
        if (typeof debugPayload === "string") {
            setDebugTextWithCopySource(resultBlock, debugPayload, DEBUG_PANEL_MAX_DISPLAY_CHARS);
            return;
        }

        setDebugTextWithCopySource(resultBlock, JSON.stringify(debugPayload, null, 2), DEBUG_PANEL_MAX_DISPLAY_CHARS);
    }
    catch {
        setDebugTextWithCopySource(resultBlock, String(llmResult), DEBUG_PANEL_MAX_DISPLAY_CHARS);
    }
}
...
function resetQueryView(clearConversation = false) {
    if (clearConversation) {
        ...
        if (keywordPromptBlock) {
            ...
            keywordPromptBlock.removeAttribute("data-copy-full-text");
        }
        if (keywordBlock) {
            keywordBlock.textContent = "-";
            keywordBlock.removeAttribute("data-copy-full-text");
        }
        if (resultBlock) {
            resultBlock.textContent = "-";
            resultBlock.removeAttribute("data-copy-full-text");
        }
        if (llmLogErrorList) {
            llmLogErrorList.textContent = "-";
            llmLogErrorList.removeAttribute("data-copy-full-text");
        }
...
function appendChatReplyMessage(reply) {
    const normalizedReply = normalizeEscapedLineBreaks(reply || "-");
    const safeReply = escapeHtml(normalizedReply).replaceAll("\n", "<br />");
...
async function loadConversationHistoryFromPanel(fileName) {
    ...
    setCurrentPolicyInsureTypeList(response.insure_type_list || response.insureTypeList || []);
...
function populateConversationFromHistory(conversations) {
    ...
    for (const conversation of items) {
        ...
        const reply = normalizeEscapedLineBreaks(((conversation === null || conversation === void 0 ? void 0 : conversation.reply) || "").toString().trim());
        if (ask) {
...
function initializePage() {
    ...
    const tryQueryByPolicySelection = async (policyButton) => {
        ...
        setCurrentPolicyInsureTypeList(policy.insureTypeList);
